...
def get_econs(day:str):
    ...
    if day == "Yesterday":
        url=f'https://financialmodelingprep.com/api/v3/economic_calendar?from={yesterday}&to={today}&apikey={API}'
    elif day == "Today":
        url=f'https://financialmodelingprep.com/api/v3/economic_calendar?from={today}&to={tomorrow}&apikey={API}'
    elif day == "Tomorrow":
        url=f'https://financialmodelingprep.com/api/v3/economic_calendar?from={tomorrow}&to={dayafter}&apikey={API}'
                ...
                if dicto[key] is not None:
                    if key == "impact":
                        ...
                        if key == "country":
                            ...
                            date_format = datetime.strptime(dicto[key],
                                                "%Y-%m-%d %H:%M:%S")
...
def get_13f_firms(name:str):
    ...
    url = f'https://financialmodelingprep.com/api/v3/cik-search/{fixstr}?apikey={API}'
    namelist = get_jsonparsed_data(url)
    return namelist

def get_cik_name(cik:str):
    url = f'https://financialmodelingprep.com/api/v3/cik/{cik}?apikey={API}'
    data = get_jsonparsed_data(url)
    name = data[0]["name"]
    return name

def get_13f_dates(cik:str):
    url = f'https://financialmodelingprep.com/api/v3/form-thirteen-date/{cik}?apikey={API}'
    file_dates = get_jsonparsed_data(url)
    print(file_dates)
    return file_dates

def get_13f_latest(cik:str, date:str):
    url = f'https://financialmodelingprep.com/api/v3/form-thirteen/{cik}?date={date}&apikey={API}'
